...
public class HttpClientBackup implements AutoCloseable {
	 ...
	 */
	public HttpClientBackup(ConnectionPolicy... policies) {
	 ...
	 * @param policies The connection policies.
	 */
	public HttpClientBackup(HttpProxy proxy, ConnectionPolicy... policies) {
	 ...
	 * @param policies The connection policies.
	 */
	public HttpClientBackup(Browser browser, ConnectionPolicy... policies) {
	 ...
	 * @param policies The connection policies.
	 */
	public HttpClientBackup(Browser browser, HttpProxy proxy, ConnectionPolicy... policies) {
	 ...
	 * @param user The user.
	 */
	public HttpClientBackup initProxies(String user) {
		this.user = user;
	...
	private Headers prepareHeaders(Request request) {
		...
		if (!request.hasBody()) {
			headers.add("Upgrade-Insecure-Requests", "1"); //TODO always or only on post?
			headers.add("Cache-Control", "max-age=0"); //TODO always?
		}
		...
		if (request.isXMLHttpRequest() || request.hasBody()) {
	 ...
	 * @return The client.
	 */
	public HttpClientBackup setLastReferer(String lastReferer) {
